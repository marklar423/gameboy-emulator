...
void writeLocation(Hardware *hardware, unsigned char *location, unsigned char value) {
	...
	else if (location == &hardware->soundData->chan1_VolumeSweep) {
		*location = value;
		hardware->soundData->chan1_VolumeCounter = -1;
	}
	else if (location == &hardware->soundData->chan2_VolumeSweep) {
		*location = value;
		hardware->soundData->chan2_VolumeCounter = -1;
	}
	...
	else if (location == &hardware->soundData->chan4_VolumeSweep) {
		*location = value;
		hardware->soundData->chan4_VolumeCounter = -1;
	}
